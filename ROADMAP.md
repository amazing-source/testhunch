...
- [ ] Lire le jeu de données : `dataset.csv` pour les métadonnées, les tables de classes pour `testclass`, `duration` et `outcome`, les comparaisons de commits pour les chemins modifiés. Jamais `last_outcome` ni leur champ `first_failure`, tous deux calculés par projet et par étage sans traiter les chevauchements
- [ ] Un seul rejeu, inscrit au registre avant de commencer, et la page publiée quel qu'en soit le résultat
- [ ] Vérifier les affirmations du papier sur nos propres chiffres plutôt que les reprendre : son README annonce 32 199 builds quand le `dataset.csv` distribué en contient 34 645
